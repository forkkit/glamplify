...
func Test_Context_AddGet(t *testing.T) {
	...
	ctx = gcontext.AddRequestFields(ctx, gcontext.RequestScopedFields{
		TraceID: "trace1",
		RequestID: "request1",
		CorrelatonID: "correlation1",
		CustomerAggregateID: "cust1",
		UserAggregateID: "user1",
	})

	rsFields, ok := gcontext.GetRequestScopedFieldsFromCtx(ctx)
	...
	assert.Assert(t, rsFields.CorrelatonID == "correlation1", rsFields)
...
func Test_Context_TraceID_AddGet_Empty(t *testing.T) {
	...
	assert.Assert(t, rsFields.CorrelatonID == "", rsFields)
...
func Test_Context_Wrap(t *testing.T) {
	...
	rsFields, ok := gcontext.GetRequestScopedFieldsFromCtx(ctx)
	assert.Assert(t, ok && rsFields.TraceID != "", rsFields)
}
